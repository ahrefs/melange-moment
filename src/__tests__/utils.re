...
module MakeTests = (M: MomentRe.MomentRe) => {
  open M;
        ...
        () => {
          ...
          test("instantiation with date", () =>
            expect(
              Moment.isSame(
                momentWithDate(
                  Js.Date.fromString("6 Mar 2017 21:22:23 GMT"),
                ),
          ...
          test("#isSame", () =>
            expect(
              Moment.isSame(moment("2016-01-01"), moment("2016-01-01")),
            )
            |> toBe(true)
          );
          test("#isBefore", () =>
          ...
          test("#day", () =>
            expect(moment("2017-01-02 03:04:05.678") |> Moment.day)
            |> toBe(1)
          );
          test("#date", () =>
          ...
          test("#startOf week", () => {
            ...
            expect(
              Moment.isSame(expected, Moment.startOf(`week, inputDate)),
            )
            |> toBe(true);
          });
          ...
          test("#endOf isoWeek", () => {
            ...
            expect(
              Moment.isSame(expected, Moment.endOf(`isoWeek, inputDate)),
            )
            |> toBe(true);
          });
          ...
          test("#momentUtc no Z (format defined)", () => {
            ...
            expect(
              momentUtc(~format=[|format|], dateStr)
              |> Moment.format(format),
            )
            |> toBe(dateStr);
          });
    ...
    describe(
      ...
      ExpectJs.(
        () => {
          ...
          test("#humanize", () =>
            expect(duration(2., `days) |> Duration.humanize)
            |> toBe("2 days")
          );
        }
      ),
